...
async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  const raw = Array.isArray(req.query.categoryId) ? req.query.categoryId[0] : req.query.categoryId ?? '';
  const categoryId = raw.toUpperCase();

  if (!/^(A|ML|LLM)\d{2}$/i.test(categoryId)) {
    res.status(400).json({ error: 'Invalid category ID', code: 'VALIDATION_ERROR' });
    return;
  }
