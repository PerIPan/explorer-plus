...
export async function GET(
  ...
  const { attackId: rawId } = await params;
  const decodedId = decodeURIComponent(rawId);
  if (!decodedId || decodedId.length > 200) {
    return withCors(errorResponse(400, 'Missing or invalid identifier', 'VALIDATION_ERROR'));
  }

  const rawParams: Record<string, string> = {};
  req.nextUrl.searchParams.forEach((v, k) => { rawParams[k] = v; });
  const limitParsed = querySchema.safeParse(rawParams);
  ...
  const idParsed = attackIdSchema.safeParse(decodedId);
  let entity: EntityRow | null = null;
  if (idParsed.success) {
    entity = await findEntity(idParsed.data);
  }
  if (!entity) {
    entity = await findExternalActor(decodedId);
  }

  if (!entity) {
    return withCors(errorResponse(404, 'Entity not found', 'NOT_FOUND'));
  }
  ...
  return withCors(jsonResponse({ center, nodes, edges, truncated }, 3600));
}
