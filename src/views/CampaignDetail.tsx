  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        titleAction={
          <Link href={`/?entity=${data.attackId}&tab=actor`} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold text-[var(--accent-orange)] underline underline-offset-2 hover:decoration-2 transition-all link-360-pulse" title="View in 360 Views">360 View →</Link>
        }
        actions={
          <div className="flex items-center gap-2">
            {(data.isRevoked || data.isDeprecated) && (<DeprecatedBadge isRevoked={data.isRevoked} />)}
            <span className="font-mono text-xs text-[var(--accent-blue)] bg-[var(--blue-faint)] border border-[var(--blue-dim)] px-2 py-1 rounded">{data.attackId}</span>
          </div>
        }
      />
