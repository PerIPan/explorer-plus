...
const columns: ColumnDef<PackageListEntry>[] = [
  ...
  },
  {
    key: 'severities',
    header: 'Severities',
    tooltip: 'Distinct severity levels across active advisories',
    width: '180px',
    render: (row) =>
      row.severities.length > 0 ? (
        <div className="flex flex-wrap gap-1">
          {row.severities.map((s) => (
            <Badge key={s} label={s} variant={SEVERITY_VARIANTS[s] ?? 'neutral'} />
          ))}
        </div>
      ) : (
  ...
  },
  {
    key: 'techniqueCount',
    header: 'Techniques',
    tooltip: 'ATT&CK techniques reachable via advisories\' CWE→CAPEC bridge',
    width: '110px',
    align: 'center',
    render: (row) => (
