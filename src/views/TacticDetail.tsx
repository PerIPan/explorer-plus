  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        titleAction={
          <Link href={`/?entity=${data.attackId}&tab=tactic-map`} className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold text-[var(--accent-orange)] underline underline-offset-2 hover:decoration-2 transition-all link-360-pulse" title="View in 360 Views">360 View →</Link>
        }
        actions={
          <span className="font-mono text-xs text-[var(--accent-yellow)] bg-[var(--yellow-faint)] border border-[var(--yellow-dim)] px-2 py-1 rounded">{data.attackId}</span>
        }
      />
